...
public class Disc extends Product {
	...
	public void addComment(Comment comment) {
		if(comment.getText() == null) {
			Comment comment2 = new Comment("",comment.getRating(),comment.getDate());
	...
	public double getAverageRating() {
		double anzahl = comments.size();
		double wertung = 0;
		for (Comment c:comments) {
			wertung += c.getRating();
		}
		double gerundet = Math.round((wertung/anzahl)*100.0)/100.0;
		if(anzahl!=0) {return gerundet;}
		else {return 0;}
	}
}
